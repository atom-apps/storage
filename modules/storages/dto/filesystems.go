...
type FilesystemForm struct {
	...
	Type      consts.Filesystem         `form:"type" json:"type,omitempty"`             // 类型
	ParentID  uint64                    `form:"parent_id" json:"parent_id,omitempty"`   // 父级ID
	Status    consts.FileStatus         `form:"status" json:"status,omitempty"`         // 状态
...
type FilesystemListQueryFilter struct {
	...
	Type      *consts.Filesystem         `query:"type" json:"type,omitempty"`             // 类型
	ParentID  *uint64                    `query:"parent_id" json:"parent_id,omitempty"`   // 父级ID
	Status    *consts.FileStatus         `query:"status" json:"status,omitempty"`         // 状态
...
type FilesystemItem struct {
	...
	Type      consts.Filesystem         `json:"type,omitempty"`       // 类型
	ParentID  uint64                    `json:"parent_id,omitempty"`  // 父级ID
	Status    consts.FileStatus         `json:"status,omitempty"`     // 状态
